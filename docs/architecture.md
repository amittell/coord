...
v0.26 layers an age-based anti-starvation boost on top of the v0.25 priority order. `db.pop_next_waiting_queue_entry` extends the `CASE` expression with a second branch keyed on `enqueued_at`: a waiting row whose age exceeds `COORD_QUEUE_AGE_BOOST_SECONDS` (default 60s) is treated as one priority level higher for ordering purposes only. The boost is recomputed on every pop -- no separate sweep, no extra writes, no change to the row's stored `priority` value -- so a long-waiting `normal` entry inherits a transient `high`-like rank that disappears the moment it is granted. Set to `0` to disable and restore the strict v0.25 priority order. v0.26 also formalises queue cancellation as an in-band lifecycle transition: `DELETE /requests/{queue_id}` flips a `waiting` row to `cancelled` and wakes its in-process long-poll immediately, so the requester's `claim_files` call returns rather than waiting out the deadline. The full lifecycle is now `waiting -> in_progress -> granted | expired | cancelled`; cancelled entries are skipped on the next pop so the head of the queue auto-advances.

v0.27 sits the new webhook outbox alongside the v0.9 `request_events` audit log rather than reusing it. Every emitted event (`claim_granted`, `auto-coexist`, `auto-narrow`, `auto-promote`, `auto-promote-subtree`, `auto-demote`, `queue_grant`, `queue_cancel`) writes the same `request_events` row it has always written; the audit log is the unconditional, durable record of what happened. `Service.fire_webhook` then *additionally* enqueues a row in the schema-v13 `webhook_outbox` table when `COORD_WEBHOOK_URL` is set and the event passes the `COORD_WEBHOOK_EVENTS` allowlist. A background delivery loop (gated on the same URL setting in `main.py`'s lifespan) POSTs each due outbox row with an HMAC-SHA256 signature header, retries with exponential backoff up to `COORD_WEBHOOK_MAX_RETRIES`, and marks the row exhausted on retry-cap. Disabling webhooks (unsetting the URL) does not change auditing -- it just stops pushing. Operators who want a durable replay always have it via the audit log; the outbox is for receivers that need real-time delivery.

v0.28 layers a small queue QoS pass on top of the v0.25 priority + v0.26 age boost model without touching the schema. `db.pop_next_waiting_queue_entry` now extends its `CASE` expression with a second age branch keyed on `enqueued_at` that *decays* effective priority one level per `COORD_QUEUE_PRIORITY_DECAY_SEC` seconds (default 300; floor at `low`), giving the v0.26 boost a symmetric counterpart and preventing a misclassified `blocking` waiter from monopolising the head of the queue. The same pop helper additionally maintains an in-memory per-blocking-claim counter and every `COORD_QUEUE_FAIRNESS_INTERVAL`-th call (default 10) bypasses the priority CASE entirely to pop by raw FIFO position, guaranteeing low-priority waiters eventually win against a steady stream of high-priority traffic. On the response path, a small FastAPI middleware reads the request's engineer signal (`X-Coord-Engineer` header or `engineer` query param), counts that engineer's `claim_queue` waiting rows, and attaches the count as the `X-Coord-Queue-Depth` response header so clients can self-regulate without a second round trip; `COORD_BACKPRESSURE_HEADER=false` disables it. Finally, `db.list_stale_engineers` returns engineers whose most-recent claim activity is older than `COORD_STALE_ENGINEER_DAYS` (default 7), powering both a "stale engineers" dashboard panel and a new `coord engineers stale [--release]` CLI subcommand that drops their lingering active claims -- the v0.28 answer to abandoned-worktree housekeeping. None of these features add tables, columns, or background loops; they are purely query-shape and middleware additions.
