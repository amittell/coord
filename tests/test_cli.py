...
def test_init_claude_creates_repo_files(
    ...
    # v0.8.1 widened the rule from .coordination/local.env to the whole
    # directory. See test_init_writes_wide_coordination_gitignore_rule.
    assert "/.coordination/" in gitignore_text
...
def test_init_writes_wide_coordination_gitignore_rule(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """End-to-end: ``coord init`` must write the wide
    ``/.coordination/`` gitignore rule, not the narrow
    ``.coordination/local.env``. A narrow rule lets ``git stash -u``
    sweep up untracked files in the .coordination dir during
    stash/pop cycles, which can leave a partial-install state where
    the hook shim exec's a missing target. The whole directory is
    per-developer state generated by ``coord init``; nothing in it
    should ever be committed, so the wide rule is correct on its
    own merits and prevents the stash failure mode by design."""
    repo = _make_repo(tmp_path)
    monkeypatch.chdir(repo)
    monkeypatch.setenv("COORD_HOME", str(tmp_path / ".coord-home"))
    ...
    assert "/.coordination/" in gitignore
    # The narrow rule (literal line) must not appear; the wide rule
    # supersedes it.
