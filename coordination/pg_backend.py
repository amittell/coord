...
# the newest entry in ``MIGRATIONS``, generated by introspecting a
# freshly-migrated in-memory SQLite database and translating its DDL.
# Generating it from the real migration chain guarantees parity instead of a
# hand-maintained copy that could drift, and the version stamp is derived
# from the same chain so it always matches the DDL actually baked into the
# body (see :func:`_full_schema_sql`).
# ---------------------------------------------------------------------------
_SCHEMA_BODY: str | None = None


def _latest_migration_version() -> int:
    """Highest version in the ``MIGRATIONS`` chain -- what a fresh bootstrap
    stamps and what :meth:`PostgresStore._apply_pending_migrations` upgrades
    an existing schema to."""
    from coordination.db import MIGRATIONS
...
def _translate_ddl(sql: str) -> str:
    ...
    The only incompatibility in the consolidated schema is the ``BOOLEAN``
    affinity on ``claims.ttl_shortened`` / ``claims.narrowable``:
...
# Idempotency rewrites for migration statements replayed against a live PG
# schema. Historical PG bootstraps stamped a hardcoded version 18 while the
# consolidated body already contained later DDL (e.g. migration 19's
# ``engineer_tokens.repo`` folded into the introspected CREATE TABLE), so the
# first migration run against such a schema re-applies DDL that is already
# physically present. ``IF NOT EXISTS`` turns those re-applies into no-ops
# instead of duplicate_column / duplicate_table / duplicate_object errors
# that would abort startup on every restart.
_ADD_COLUMN_RE = re.compile(
    r"(\bADD\s+COLUMN\s+)(?!IF\s+NOT\s+EXISTS\b)", re.IGNORECASE
)
_CREATE_TABLE_RE = re.compile(
    r"(\bCREATE\s+TABLE\s+)(?!IF\s+NOT\s+EXISTS\b)", re.IGNORECASE
)
...
def _translate_migration_stmt(stmt: str) -> str:
    """Translate one SQLite migration statement for execution on PG: map the
    BOOLEAN affinity (:func:`_translate_ddl`) and make the DDL shapes used by
    the migration chain (``ADD COLUMN`` / ``CREATE TABLE`` / ``CREATE
    INDEX``) idempotent via ``IF NOT EXISTS`` so a statement whose effect is
    already baked into the consolidated bootstrap body no-ops instead of
    crashing init."""
    sql = _translate_ddl(stmt)
    sql = _ADD_COLUMN_RE.sub(lambda m: m.group(1) + "IF NOT EXISTS ", sql)
    sql = _CREATE_TABLE_RE.sub(lambda m: m.group(1) + "IF NOT EXISTS ", sql)
    sql = _CREATE_INDEX_RE.sub(lambda m: m.group(1) + "IF NOT EXISTS ", sql)
    return sql


def _build_schema_body() -> str:
    """Generate the consolidated current-version DDL by replaying the real
...
def _full_schema_sql(body: str) -> str:
    ...
    consolidated tables/indexes, the leader-lease table, and a version stamp
    equal to the newest migration replayed into ``body`` -- derived, never
    hardcoded, so the stamp cannot drift below the DDL it labels (a lower
    stamp would make :meth:`PostgresStore._apply_pending_migrations`
    re-apply DDL the body already contains)."""
    return (
...
def _t_strftime(inner: str) -> str:
    ...
    if fmt == "'%Y-%m-%d'":
        # ``to_char(timestamptz, ...)`` renders in the session TimeZone, but
        # stored timestamps are UTC ISO-8601 text and SQLite's strftime
        # buckets by UTC date. ``AT TIME ZONE 'UTC'`` converts to a UTC-local
        # timestamp first so the day bucket matches the SQLite backend on any
        # server default timezone (the session is also pinned to UTC in
        # ``_set_search_path``; this keeps the SQL correct standalone).
...
class PostgresStore(Database):
    ...
    def __init__(
        ...
        *,
        ...
        # AttributeError on the None asyncpg module.
        if asyncpg is None:
            raise RuntimeError(
                "COORD_DATABASE_URL selects the PostgreSQL backend but the "
                "'asyncpg' driver is not installed. Install the postgres "
        ...
        async with self._init_lock:
            ...
            async with pool.acquire() as raw:
                async with raw.transaction():
                    ...
                    # two processes never race to create the same schema or
                    # to run migrations. The lock must be taken BEFORE the
                    # CREATE SCHEMA: ``IF NOT EXISTS`` is a non-atomic
                    # check-then-create in PostgreSQL, so two unserialized
                    # sessions can both pass the existence check and one then
                    # dies on the pg_namespace unique index. The advisory key
                    # is hashed from text and needs no schema to exist.
                    await raw.execute(
                    ...
                    if exists is None:
                        ...
                        # Already bootstrapped by an earlier build: bring the
                        # schema up to the current migration chain, exactly
                        # like the SQLite _migrate_to_current path. Without
                        # this an existing PG deployment never gains new
                        # columns (e.g. migration 19's engineer_tokens.repo)
    ...
    async def _apply_pending_migrations(self, raw: asyncpg.Connection) -> None:
        ...
        schema_version, then stamp each version as it lands -- the PG
        equivalent of :func:`coordination.db._migrate_to_current`. Runs
        inside init's advisory-locked transaction, so concurrent replicas
        serialize and a failure rolls the whole batch back.

        Statements go through :func:`_translate_migration_stmt`, which makes
        the DDL idempotent (``IF NOT EXISTS``). That matters for schemas
        bootstrapped by builds that stamped a hardcoded version 18 while the
        consolidated body already contained later DDL: re-applying such a
        migration must no-op, not raise duplicate_column and brick startup.
        """
        from coordination.db import MIGRATIONS, _split_sql_statements

        version_row = await raw.fetchval(
            "SELECT version FROM schema_version WHERE id = 1"
        )
        current = int(version_row) if version_row is not None else 0
        latest = _latest_migration_version()
        if current > latest:
            raise RuntimeError(
                f"database schema_version={current} is from a newer version "
                f"of coord (expected <= {latest}); refusing to start. "
                "Upgrade coord or restore an older database."
            )
        pending = sorted((v, sql) for (v, sql) in MIGRATIONS if v > current)
        for version, upgrade_sql in pending:
            for stmt in _split_sql_statements(upgrade_sql):
                await raw.execute(_translate_migration_stmt(stmt))
            await raw.execute(
    ...
    async def _set_search_path(self, raw: asyncpg.Connection) -> None:
        # Pin the session timezone alongside the search_path: stored
        # timestamps are UTC ISO-8601 text, and the translated time
        # expressions (text::timestamptz casts, to_char day buckets) must
        # evaluate in UTC exactly as SQLite does, regardless of the PG
        # server's default TimeZone. One round trip: asyncpg runs multi-
        # statement scripts through the simple query protocol when no
