"""Tree-sitter backed JavaScript symbol extractor.

This backend walks the top level of the ``program`` node produced by the
``tree-sitter-javascript`` grammar and emits one :class:`Symbol` per
claimable declaration. JavaScript shares most of its declaration structure
with TypeScript, so this mirrors ``ts_treesitter`` minus the type-system
constructs JavaScript does not have (interfaces, type aliases, enums, type
annotations).
...
- ``lexical_declaration`` (``const``/``let``) / ``variable_declaration``
  (``var``) -- ``kind='const'`` when the binding is a ``function_expression``,
  ``generator_function`` or ``arrow_function``. Other bindings are ignored
  (``const x = 42`` is not a callable, so it is not a unit of coordination
  for sub-file claims).
...
v0.16 emits method-level symbols nested inside a ``class_declaration``. Each
direct child of the class's ``class_body`` is examined and emitted with
``parent=<class name>``:

- ``method_definition`` -> ``kind='function'`` (covers regular methods, the
  ``constructor``, static methods, async methods, generator methods, getters
  and setters; the ``get``/``set`` distinction is collapsed for v0.16).
- ``field_definition`` whose value is an ``arrow_function``,
  ``function_expression`` or ``generator_function`` -> ``kind='const'``.

v0.19 extends the class walk to RECURSE into nested classes. JavaScript class
bodies cannot contain a bare ``class Inner {}`` declaration, but the realistic
shape ``static Inner = class Inner { handle() {} }`` binds a class expression
to a field. The inner class itself emits with ``parent=<outer class name>``
(or the full ancestor path for deeper nesting), and its members emit with the
full path joined by ``"::"``. So
``class Outer { static Inner = class { handle() {} } }`` produces:
...
changes. Classes nested inside a method body (closures) are still excluded;
...
def _js_parser():
    """Return a cached tree-sitter JavaScript parser instance."""

    global _parser_js
    if _parser_js is not None:
        return _parser_js
    import tree_sitter_javascript as ts_js
    from tree_sitter import Language, Parser

    language = Language(ts_js.language())
    _parser_js = Parser(language)
    return _parser_js
...
def _function_like(node) -> bool:
    """True when ``node`` is an arrow function, function or generator expression."""
...
def _symbol_from_named_declaration(node, is_default: bool) -> Symbol | None:
    ...
    if name_node is None:
        if is_default and node.type in {
            "function_declaration",
            "generator_function_declaration",
        }:
            # ``export default function() {}`` parses as a function expression,
            # not a declaration, so this branch only triggers for forms
            # tree-sitter cannot otherwise name.
            return Symbol(
                name="default",
                kind="function",
...
def _symbols_from_variable_declaration(node) -> list[Symbol]:
    ...
    a function, generator or arrow expression are emitted; ``const NotFunc =
    42`` is silently dropped because it is not a callable scope.
    """
...
def _method_name(node) -> str | None:
    ...
    ``method_definition`` exposes its name through the ``name`` field, while
    ``field_definition`` exposes it through the ``property`` field in the
    ``tree-sitter-javascript`` grammar; we try ``name`` first and fall back to
    ``property`` so both member shapes resolve. Computed property names
    (``[Symbol.iterator]() {}``) are not plain ``property_identifier`` nodes
    and are skipped -- they have no static name addressable from a claim. The
    constructor is named ``constructor`` per the grammar.
    """
...
def _walk_class(
    ...
    syntactic site provides a binding name distinct from (or in place of) the
    class expression's own name. The realistic JS pattern for a nested class
    is ``static Inner = class Inner { ... }``; the binding identifier
    ``Inner`` is the addressable name. When ``name_override`` is set, it wins
    over the class expression's optional name.

    ``span_node`` lets the emitted Symbol span the enclosing site (so the
    ``static Inner = class { ... };`` field definition's line range is used)
    instead of just the class body. ``None`` falls back to ``class_node``.

    The returned list always starts with this class's own symbol, followed by
    its members in source order. Class expressions inside field values recurse
    depth-first.

    ``method_definition`` (regular methods, ``constructor``, static, async,
    generator, getters, setters) emit with ``kind='function'`` and
    ``parent=full_path``. ``field_definition`` whose value is an
    arrow/function/generator expression emit with ``kind='const'`` and
    ``parent=full_path``; whose value is a class expression recurse into
    :func:`_walk_class` with the field's binding name as the addressable
    identifier. Other class members (plain property declarations, static
    blocks) are not callable scopes and are skipped.
    """
    ...
    for member in body.children:
        ...
        # tree-sitter-javascript grammar (the grammar treats ``class X {}``
        # inside a class body as a syntax error), but the branch is kept for
        # forward compatibility with grammar evolutions or alternative
        ...
        # JavaScript class fields surface as ``field_definition``. Older or
        # divergent grammar revisions used ``public_field_definition`` (the
        # TypeScript spelling); accept both so this backend is robust to the
        # wheel version installed.
        if member.type in {"field_definition", "public_field_definition"}:
            ...
            if name is None:
                continue
            # Class expression on the right-hand side: recurse so the nested
            # class is addressable via its binding name. This is the realistic
            # JS shape for nested classes.
        ...
        # Other class members (plain property declarations without a function
        # value, static initialisation blocks) are not callable scopes and are
        # skipped.
...
def extract(content: str) -> list[Symbol]:
    ...
    Top-level declarations (functions, generator functions, classes,
    ...
    for child in root.children:
        ...
        if decl.type in _DECLARATION_KINDS:
            ...
            # order. Other named declarations (plain/generator functions)
